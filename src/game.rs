use chess::{ChessMove, Color, Game, MoveGen, Piece, Square, Rank, File, Board};
...
impl ChessGame {
    ...
    pub fn set_player_color(&mut self, color: Color) {
        ...
        self.message = format!("You are playing as {}.", if color == Color::White { "White" } else { "Black" });
    }
    ...
    pub fn select_square(&mut self, square: Square) -> bool {
        ...
        if let Some(_selected) = self.selected_square {
            ...
            let possible_move = self.possible_moves.iter().find(|m| m.get_dest() == square).cloned();
